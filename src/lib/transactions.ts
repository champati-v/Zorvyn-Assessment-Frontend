...
export type DateRangeKey = "all" | "month" | "quarter" | "half" | "year";
...
const rangeMonths: Record<Exclude<DateRangeKey, "all">, number> = {
  month: 1,
  quarter: 3,
  half: 6,
  year: 12,
...
export function sortTransactionsDesc(transactions: Transaction[]) {
...
export function getMonthlyBalanceSeries(transactions: Transaction[], monthsBack = 6) {
  ...
  transactions
    ...
    .forEach((transaction) => {
      breakdown.set(
        ...
        (breakdown.get(transaction.category) ?? 0) + Math.abs(transaction.amount)
      );
    });
...
export function filterTransactionsByRange(
  ...
  const startDate = startOfMonth(
    new Date(latestDate.getFullYear(), latestDate.getMonth() - (months - 1), 1)
  );
  const endDate = endOfMonth(latestDate);

  return transactions.filter((transaction) =>
    transaction.date >= format(startDate, "yyyy-MM-dd") &&
